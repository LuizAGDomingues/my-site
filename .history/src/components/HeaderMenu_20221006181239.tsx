...
function HeaderMenu({refs}: HeaderProps) {
  return (
    <div className='h-12 bg-zinc-900 text-white rounded-md fixed top-0 animate-moveHeaderMenu'>
      <DropdownMenu.Root>
        <DropdownMenu.Trigger>
          <List color="white" weight="fill" className='m-2 h-8 w-8 hover:opacity-70' />
        </DropdownMenu.Trigger>
        <DropdownMenu.Portal>
          <DropdownMenu.Content className='bg-zinc-900 text-white w-40 h-48 rounded-md'>
            <DropdownMenu.Item >
              <Link href='/'>
